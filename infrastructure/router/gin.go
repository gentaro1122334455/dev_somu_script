...
func (g ginEngine) setAppHandlers(router *gin.Engine) {
	...
	router.POST("/v1/userdata", g.buildCreateUserDataAction())
	router.GET("/v1/health", g.healthcheck())
}

func (g ginEngine) buildCreateUserDataAction() gin.HandlerFunc {
	return func(c *gin.Context) {
		var (
			uc = usecase.NewCreateUserDataInteractor(
				repository.NewUserDataNoSQL(g.db),
				presenter.NewCreateUserDataPresenter(),
				g.ctxTimeout,
			)

			act = action.NewCreateUserDataAction(uc, g.log, g.validator)
		)
